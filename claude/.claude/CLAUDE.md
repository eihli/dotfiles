...
- ❌ `# Calculate EPS surprise` (obvious)
- ✅ `# Use abs() to handle negative estimates (expected loss)`
...
- "Build a backtesting framework" → goal is backtesting, not building frameworks. Suggest existing tools?
- "Grid search for hyperparameters" → goal is optimization. Bayesian optimization? Optuna?
