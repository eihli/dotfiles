...
# Lisa Stop Hook
# Prevents session exit when a lisa is active
...
# Check if lisa is active
RALPH_STATE_FILE=".claude/lisa.local.md"
...
if [[ ! "$ITERATION" =~ ^[0-9]+$ ]]; then
  ...
  echo "   Lisa loop is stopping. Run /lisa again to start fresh." >&2
  rm "$RALPH_STATE_FILE"
  exit 0
fi
...
if [[ "$COMPLETION_PROMISE" != "null" ]] && [[ -n "$COMPLETION_PROMISE" ]]; then
  SYSTEM_MSG="🔄 Lisa iteration $NEXT_ITERATION | To stop: output <promise>$COMPLETION_PROMISE</promise> (ONLY when statement is TRUE - do not lie to exit!)"
else
  SYSTEM_MSG="🔄 Lisa iteration $NEXT_ITERATION | No completion promise set - loop runs infinitely"
fi
